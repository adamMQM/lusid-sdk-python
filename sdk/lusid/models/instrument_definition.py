    ...
    def from_dict(cls, obj: dict) -> InstrumentDefinition:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in InstrumentDefinition) in the input: " + obj)

        _obj = InstrumentDefinition.parse_obj({
            "name": obj.get("name"),
            "identifiers": {k: (None if v is None else InstrumentIdValue.from_dict(v)) for k, v in obj.get("identifiers").items()} if obj.get("identifiers") is not None else None,
