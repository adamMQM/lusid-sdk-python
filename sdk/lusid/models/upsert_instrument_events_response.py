...
class UpsertInstrumentEventsResponse(BaseModel):
    """
    ...
    @classmethod
    def from_json(cls, json_str: str) -> UpsertInstrumentEventsResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> UpsertInstrumentEventsResponse:
        """Create an instance of UpsertInstrumentEventsResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return UpsertInstrumentEventsResponse.parse_obj(obj)

        # raise errors for additional fields in the input
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in UpsertInstrumentEventsResponse) in the input: " + obj)

        _obj = UpsertInstrumentEventsResponse.parse_obj({
            "href": obj.get("href"),
            "values": {k: (None if v is None else InstrumentEventHolder.from_dict(v)) for k, v in obj.get("values").items()} if obj.get("values") is not None else None,
