...
class SetLegalEntityIdentifiersRequest(BaseModel):
    ...
    def from_dict(cls, obj: dict) -> SetLegalEntityIdentifiersRequest:
        ...
        if not isinstance(obj, dict):
            return SetLegalEntityIdentifiersRequest.parse_obj(obj)

        # raise errors for additional fields in the input
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in SetLegalEntityIdentifiersRequest) in the input: " + obj)

        _obj = SetLegalEntityIdentifiersRequest.parse_obj({
            "identifiers": {k: (None if v is None else ModelProperty.from_dict(v)) for k, v in obj.get("identifiers").items()} if obj.get("identifiers") is not None else None,
        })
        return _obj
