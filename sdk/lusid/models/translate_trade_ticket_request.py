    ...
    def from_dict(cls, obj: dict) -> TranslateTradeTicketRequest:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in TranslateTradeTicketRequest) in the input: " + obj)

        _obj = TranslateTradeTicketRequest.parse_obj({
            "tickets": {k: (None if v is None else TradeTicket.from_dict(v)) for k, v in obj.get("tickets").items()} if obj.get("tickets") is not None else None,
