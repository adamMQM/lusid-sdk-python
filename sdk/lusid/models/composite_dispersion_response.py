    ...
    def from_dict(cls, obj: dict) -> CompositeDispersionResponse:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in CompositeDispersionResponse) in the input: " + obj)

        _obj = CompositeDispersionResponse.parse_obj({
            "href": obj.get("href"),
            "results": {k: (None if v is None else [CompositeDispersion.from_dict(iv) for iv in v]) for k, v in obj.get("results").items()} if obj.get("results") is not None else None,
