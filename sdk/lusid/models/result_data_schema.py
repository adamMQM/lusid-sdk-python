    ...
    def from_dict(cls, obj: dict) -> ResultDataSchema:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in ResultDataSchema) in the input: " + obj)

        _obj = ResultDataSchema.parse_obj({
            "node_value_schema": {k: (None if v is None else FieldSchema.from_dict(v)) for k, v in obj.get("nodeValueSchema").items()} if obj.get("nodeValueSchema") is not None else None,
            "property_schema": {k: (None if v is None else FieldSchema.from_dict(v)) for k, v in obj.get("propertySchema").items()} if obj.get("propertySchema") is not None else None,
            "address_schema": {k: (None if v is None else AddressDefinition.from_dict(v)) for k, v in obj.get("addressSchema").items()} if obj.get("addressSchema") is not None else None,
        })
        return _obj
