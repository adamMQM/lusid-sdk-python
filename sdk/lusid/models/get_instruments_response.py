...
class GetInstrumentsResponse(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GetInstrumentsResponse:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GetInstrumentsResponse:
        """Create an instance of GetInstrumentsResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GetInstrumentsResponse.parse_obj(obj)

        # raise errors for additional fields in the input
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in GetInstrumentsResponse) in the input: " + obj)

        _obj = GetInstrumentsResponse.parse_obj({
            "href": obj.get("href"),
            "values": {k: (None if v is None else Instrument.from_dict(v)) for k, v in obj.get("values").items()} if obj.get("values") is not None else None,
